...
If you use Jupyter notebook version < 5.3, you need an extra step to [enable ipywidgets](http://ipywidgets.readthedocs.io/en/latest/user_install.html) along with other libraries  (using `--user` instead of `--sys-prefix` if doing a local install):
